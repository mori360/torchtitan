    ...
    if parent_cls not in _lora_class_cache:
        ...
        class LoRALinear(parent_cls):  # type: ignore[valid-type, misc]
            ...
            def named_parameters(
                ...
                # Expose only direct parameters (weight, bias) so that
                # ColwiseParallel._partition_linear_fn does not encounter
                # dotted submodule names like "lora_a.weight" and crash.
                # The adapter submodules (lora_a, lora_b) are still visited
                # separately by distribute_module's named_modules() traversal
                # and their weights are made Replicate automatically via
                # replicate_module_params_buffers — zero extra TP communication.
